...
class NaoBlobDetectionModule( NaoNode ):
    ...
    def onImageReceived( self, data ):
        ...
        try:
            ...
            cv.PyrMeanShiftFiltering( openCVImage, openCVImage, 5, 20 );
            #cv.Smooth( openCVImage, openCVImage, cv.CV_GAUSSIAN, 11, 11 );
                
            # Reduce colors.
            '''
            hist = cv.CreateHist( [32], CV_HIST_ARRAY, [[0,256]], 1 );
            cv.CalcHist([hue_channel, sat_channel], hist, 0, None );
            '''
            ...
            step = openCVImage.step;
            data = openCVImage.data;
            
            for i in xrange( 0, width ):
                for j in xrange( 0, height ):
                    c1 = ( data[ i * step + j * 3 + 0 ] & 192 ) >> 2;
                    c2 = ( data[ i * step + j * 3 + 1 ] & 192 ) >> 4;
                    c3 = ( data[ i * step + j * 3 + 2 ] & 192 ) >> 6;
                    
                    data[ i * step + j ] = c1 | c2 | c3;
            '''     

            # Set the message.
            ImageMessage = self.__cvBridge.cv_to_imgmsg( openCVImage );
